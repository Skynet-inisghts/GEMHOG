...
/**
 * Every JSON-RPC request gemhog makes goes through this gate.
 * Adapted from bodkin (MIT) — https://github.com/Phosphenq/bodkin
 *
 * The public endpoints this tool is written against behave differently:
 * - the official Robinhood RPC answers HTTP 429 above roughly eight concurrent
 *   calls, meters eth_getLogs much more tightly than eth_call, and hands a
 *   Cloudflare challenge page to a client that has been noisy for a while;
 * - publicnode is fast and generous for state reads but refuses eth_getLogs.
 *
 * So: a list of endpoints with capabilities, single requests (no batching),
 * bounded concurrency, minimum spacing (tighter for eth_getLogs), a
 * process-wide cooldown after a 429, and a penalty box per endpoint. A request
 * is routed to the first healthy endpoint that supports its method and moves to
 * the next one when it is refused. Set RPC_URL to a comma-separated list to
 * replace the defaults; a private provider can carry everything.
 */
...
/** Requests issued since process start; every certificate prints it as provenance. */
export const rpcCallCount = (): number => calls;

/** For status lines: how busy the gate is and how the endpoints are doing. */
