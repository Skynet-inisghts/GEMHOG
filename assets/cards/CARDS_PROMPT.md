...
## Что это
Квадратная картинка 1080×1080 по результату `check`. Персонаж — маскот из хиро в его точной форме (анфас, спина за головой, копыта по бокам, горка земли впереди), три варианта по зоне:
- 1–35 — красная, `pig-red.png`: уши повисли, тяжёлые веки, брови сведены, капля пота, грязь на щеках, в яме уголь, муха
- 36–70 — жёлтая, `pig-yellow.png`: одна бровь поднята, рот чёрточкой, правое копыто приподнято с булыжником, яма пустая
- 71–100 — зелёная, `pig-green.png`: глаза с бликами, улыбка, румянец, оба копыта подняты и держат алмаз перед пятаком, искры, в земле ещё камни

Спрайты не перерисовывать — брать PNG из `assets/cards/`; горка — часть спрайта и уходит за нижний край карточки (спрайт ставится на 24px ниже края).

Раскладка (все координаты и размеры — в `card()` внутри `render_cards.py`): сверху слева лого токена в круге с обводкой цвета зоны + `$ТИКЕР` (`$` — JetBrains Mono Bold цветом зоны, тикер — Tiny5 белым, ужимается шагом 8px, если длинный), справа скор Unbounded Black цветом зоны, слева от него `/100` серым, под скором грейд Tiny5; ниже полоса 1–100 с тремя зонами и маркером; три строки описания JetBrains Mono; персонаж 624px внизу, прижат к правому полю, свечение цвета зоны за ним; в левом нижнем углу столбик: `GEMHOG` (Tiny5, цвет зоны), `Terminal` (моно серым), `gemhog.xyz`, дата грейда. Фон чёрный.
...
- `DEMO`-сертификат даёт карточку с плашкой `DEMO` поверх персонажа, как у всего остального демо.
